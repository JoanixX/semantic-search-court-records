...
short src_ln1 [] = {
	  0,  87,  93,  95, 102, 104, 106, 107, 
	109, 110, 112, 115, 117,  90, 124, 127, 
	128,  89, 130,  89, 130,   0, };
S_F_MAP src_file1 [] = {
	{ "-", 0, 0 },
	{ "modelo_final.pml", 1, 20 },
	{ "-", 21, 22 }
};
uchar reached1 [] = {
	  0,   0,   1,   0,   0,   0,   0,   0, 
...
short src_ln0 [] = {
	  0,  50,  52,  58,  59,  61,  62,  64, 
	 67,  69,  41,  72,  73,  40,  75,  40, 
	 75,   0, };
S_F_MAP src_file0 [] = {
	{ "-", 0, 0 },
	{ "modelo_final.pml", 1, 16 },
	{ "-", 17, 18 }
};
uchar reached0 [] = {
	  0,   1,   0,   0,   0,   0,   0,   0, 
...
char *procname[] = {
   ...
   "exclusion_mutua",
   "progreso",
   "anti_starvation_worker0",
   "anti_starvation_worker1",
   "anti_starvation_worker2",
   "buffer_seguro",
   ":np_:",
	0
};
...
int Btypes[] = {
   ...
   1,	/* exclusion_mutua */
   1,	/* progreso */
   1,	/* anti_starvation_worker0 */
   1,	/* anti_starvation_worker1 */
   1,	/* anti_starvation_worker2 */
   1,	/* buffer_seguro */
   0	/* :np_: */
};
...
{	int j = 0, h = now._nr_pr;
...
#ifndef NOCLAIM
	case 9:	/* claim select */
		spin_c_typ[0] = 7; /* buffer_seguro */
		((P9 *)pptr(h))->c_cur[0] = 6;
		reached7[6]=1;
		spin_c_typ[1] = 6; /* anti_starvation_worker2 */
		((P9 *)pptr(h))->c_cur[1] = 5;
		reached6[5]=1;
		spin_c_typ[2] = 5; /* anti_starvation_worker1 */
		((P9 *)pptr(h))->c_cur[2] = 5;
		reached5[5]=1;
		spin_c_typ[3] = 4; /* anti_starvation_worker0 */
		((P9 *)pptr(h))->c_cur[3] = 5;
		reached4[5]=1;
		spin_c_typ[4] = 3; /* progreso */
		((P9 *)pptr(h))->c_cur[4] = 5;
		reached3[5]=1;
		spin_c_typ[5] = 2; /* exclusion_mutua */
		((P9 *)pptr(h))->c_cur[5] = 6;
		reached2[6]=1;
		((P9 *)pptr(h))->_t = 2;
		((P9 *)pptr(h))->_p = 6;
		((P9 *)pptr(h))->_n = 5; /* exclusion_mutua */
		src_claim = src_ln2;
#ifndef BFS
		if (whichclaim == -1 && claimname == NULL)
			printf("pan: ltl formula exclusion_mutua\n");
#endif
		if (whichclaim != -1)
...
#endif
		break;
	case 6:	/* anti_starvation_worker2 */
		((P6 *)pptr(h))->_t = 6;
		((P6 *)pptr(h))->_p = 5;
#ifdef HAS_PRIORITY
...
#endif
		break;
	case 5:	/* anti_starvation_worker1 */
		((P5 *)pptr(h))->_t = 5;
		((P5 *)pptr(h))->_p = 5;
#ifdef HAS_PRIORITY
...
#endif
		break;
	case 4:	/* anti_starvation_worker0 */
		((P4 *)pptr(h))->_t = 4;
		((P4 *)pptr(h))->_p = 5;
#ifdef HAS_PRIORITY
	...
	case 1:	/* Limpiador */
		...
		((P1 *)pptr(h))->id = 0;
#ifdef VAR_RANGES
		logval("Limpiador:id", ((P1 *)pptr(h))->id);
#endif
#ifdef HAS_CODE
		locinit1(h);
#endif
...
iniglobals(int calling_pid)
{
	...
	{	int l_in;
		for (l_in = 0; l_in < 3; l_in++)
		{
			now.worker_jobs[l_in] = 0;
		}
	}
#ifdef VAR_RANGES
		logval("mutex", now.mutex);
		logval("in_critical", now.in_critical);
	{	int l_in;
		for (l_in = 0; l_in < 3; l_in++)
		{
			logval("worker_active[l_in]", now.worker_active[l_in]);
		}
	}
		logval("buffer_count", now.buffer_count);
		logval("produced", now.produced);
		logval("consumed", now.consumed);
	{	int l_in;
		for (l_in = 0; l_in < 3; l_in++)
		{
			logval("worker_jobs[l_in]", now.worker_jobs[l_in]);
		}
	}
#endif
}
...
{	/* int i; */
	printf("global vars:\n");
	printf("	byte   buffer_count:	%d\n", now.buffer_count);
	printf("	byte   produced:	%d\n", now.produced);
	printf("	byte   consumed:	%d\n", now.consumed);
	printf("	bit    mutex:	%d\n", now.mutex);
	printf("	bit    in_critical:	%d\n", now.in_critical);
	{	int l_in;
		for (l_in = 0; l_in < 3; l_in++)
		{
			printf("	byte   worker_jobs[%d]:	%d\n", l_in, now.worker_jobs[l_in]);
		}
	}
	{	int l_in;
		for (l_in = 0; l_in < 3; l_in++)
		{
			printf("	bit    worker_active[%d]:	%d\n", l_in, now.worker_active[l_in]);
		}
	}
}
void
...
{	/* int i; */
	...
	case 2:
		...
		break;
	case 1:
		printf("local vars proc %d (Limpiador):\n", pid);
	printf("	byte   id:	%d\n", ((P1 *)pptr(pid))->id);
		break;
	case 0:
		/* none */
